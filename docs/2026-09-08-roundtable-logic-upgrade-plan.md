...
| §5.16 功能开关·人工介入（轮间暂停） | 插话链路（`interject.rs` + 轮边界 drain + UI）早已存在，但**轮循环无任何用户等待点**——只能事后补充意见，无法决定"是否再讨论一轮" | ⚠️ 2026-09-18 更正（第九批）：**已达成**。新增第三正交机制 `gate`（按 run_id 隔离的门槽 + 有上限等待 + 取消优先 + 超时放行并记 `pause_gate_degraded`），门开在**轮末**、仅在"还有下一轮"时开；请求 `round_gate` 缺省 false（老前端/无头/矩阵不暂停），设置默认开启可一键关闭；用户思考时间经 `ROUND_GATE_TIMEOUT_ALLOWANCE` 从生成预算中豁免 |
...
| localStorage 设置 | 只增字段（第九批补 `roundGate`） | 旧文件零迁移直接读：`sanitizeStored` 缺字段即默认值；写入侧同一清洗函数（`persistNonSecrets`）——**新增字段必须同时登记清洗白名单，否则落盘即丢** |
...
用户输入 → [阶段0 主持人：模式指令+checklist(新D3)+知识库] → 初稿
        ...
        → [轮末人工确认门(第九批，仅开启且还有下一轮)：暂停等决断 → 继续 / 结束讨论 / 超时自动继续]
...
每节点失败行为：阶段 0 失败=整 run 报错（既有）；讨论轮角色失败=降级继续+flag（既有+D5）；**轮末确认门超时=自动继续下一轮 + `pause_gate_degraded`（第九批；永不挂死）**；收敛失败=轮次上限强制收敛（既有）；阶段 2 打回耗尽=降级返回+flag（既有+D5）。

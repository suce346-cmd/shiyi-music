...
- **2026-09-06 第 1 批（B1/B2/B5）完成**：已推送，CI test job 验证通过（run 33986410964）。
- **2026-09-06 第 2 批（B10/B4）完成**：本地 commit `b5e3be1`/`17e63f6`。
- **2026-09-06 用户决策：max_tokens 上限统一放开至 30000**（llm.rs `MAX_TOKENS_CAP`）。
  本文档 B4 节的"3000 基线 → 6000 提额重试"阶梯方案被此决策**取代**：基线即 30000，
  截断提额重试机制简化为观测日志；解析失败重试、格式打回、阶段 0 截断报错、
  TRUNCATION_ISSUE 注入打回循环等其余分级处置全部保留。
- **2026-09-06 第 3 批（B9/B7/B6 + 上诉 token 决策）完成**：本地 commit。
  B8（Verse 逐段检查）顺延至第 4 批（单批 ≤3 个 bug 约束）。

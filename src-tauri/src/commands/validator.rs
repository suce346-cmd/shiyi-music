    ...
    if mode == "mode_a" || mode == "mode_b" {
        ...
        if !counts.is_empty() {
            let detail = counts
                ...
                .collect::<Vec<_>>()
                .join(", ");
            // 差值/上限用全量段（极简段 1-2 件正是动态对比的来源）；下限豁免只作用于 weak 检查
            ...
            if !counted.is_empty() && counted.iter().any(|(_, c)| *c < rules::MIN_INSTRUMENT_WEAK) {
                let weak: Vec<String> = counted
                    .iter()
                    .filter(|(_, c)| *c < rules::MIN_INSTRUMENT_WEAK)
                    .map(|(t, c)| format!("{}:{}件", t, c))
                    .collect();
                issues.push(format!(
...
/// 只查歌词行（is_lyric_line），说明行/标签/参数行不查。

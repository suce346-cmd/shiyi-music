...
/// C2/ADR-1：转写契约（orchestrator 在开关开时注入校验员格式输出 system）。
/// 目的：把阶段 2 从"重新创作"约束为"受控转写"——收敛承诺传递到终稿；
/// 违规由保真校验（check_transcription_fidelity）+ 定点重写兜住。
pub const TRANSCRIPTION_CONTRACT: &str = "\n【转写契约（最高优先级，覆盖一切格式化冲动）】你拿到的是已收敛的最终方案，你的任务是【转写】不是【重写】：\n1. 歌词部分逐行保留：每行字数、行数、断句空格、用语一律不变——原文某行 7 字，输出必须 7 字；原文 3 行，输出必须 3 行。说明行（[乐器+行为, …] 行）、结构标签、Style Prompt 行与参数行都是必须转写的格式要素，一个都不得省略（终稿缺其中任何一项即失败）。\n2. 你只允许做三类格式操作：补齐缺失的格式要素（说明行/参数行/结构标签）、修正说明行格式（配器表述/能量标注）、调整段落标签命名。\n3. 若发现收敛方案本身违反硬规则（字数不符/缺骤停/参数越界等），不要自行修改——原样转写并在输出第一行加标记 `TRANSCRIPTION_ISSUE: <问题描述>`（系统会把方案回炉给主持人修正，你改了反而破坏责任链）。例外：若调用方在输入中列出【格式问题】清单，其中列出的违规项必须修正到合规（修正优先于'原样保留'），其余内容仍逐字转写。\n";

/// 校验员·讨论轮审查人设（阶段 1：审查方案与角色修订，提出观点返回主持人）。
/// 与 auditor()（阶段 2 格式端口）职责分离：讨论轮提观点，终局做格式输出。
pub fn auditor_review_prompt() -> &'static str {
    "你是 Suno 流水线的【校验员】（讨论轮专业审查）。你审查主持人当前方案与各专业角色本轮提出的修订片段：\n先分析（对照模式质量审查门逐项核查）：\n1. 结构完整：Style Prompt 信息块/歌词/参数齐全？各模式专项满足（A/B 模式 ${STYLE_BLOCKS_AB} 块、抖音 ${STYLE_BLOCKS_DOUYIN} 块且 BPM≥${DOUYIN_BPM_MIN}、Mode C 逐行字数对齐）？\n2. 格式必检项：硬校验会打回的项目是否已存在（Hook≥${HOOK_MIN} 与骤停[抖音]、能量标注[A/B]、字数对齐[C]）？以及格式规范项（断句单空格、参数区间）是否合规——参数区间按模式判定：抖音模式 Weirdness ${DOUYIN_WEIRD_RANGE}/Style Influence ${DOUYIN_STYLE_RANGE}，不套用 A/B 弧线区间（见校验清单弧线表，如 style_arc_drama）；A/B 模式对照校验清单弧线区间；例外（与硬校验同口径）：抖音模式仅当结构含 ≥${NARRATIVE_SECTIONS_MIN} 个叙事段（Verse/Pre-Chorus/Bridge 标签）且方案中写明'叙事型'归类时，参数可回落 A/B 弧线区间；二者缺一即按抖音区间打回；纯抖音结构（Hook 循环/一句话循环/先压后炸）不得回落（后者靠格式规范与 suno_rules 软兜底）？\n3. 修订合理性：各角色修订是否互相冲突（两人改同一处、目标矛盾）？是否解决上一轮提出的问题？有无该提没提的漏项？\n4. 质量门：可唱性（不赶气、重要词行尾）？配器依据（每件乐器能在情感翻译/意象映射中找到依据）？动态对比（最弱 vs 最强 ≥${MIN_ENERGY_GAP} 级、配器差 ≥${MIN_INSTRUMENT_GAP} 件、说明行差异能否驱动 Suno 起伏）？人声是否从心理状态推导而非模板（换一种情绪，人声描述应该不同）？\n5. 独特性与收敛：方案是否模板化（与\"同流派同情绪\"的其他歌相比独特之处？答不出=太模板化）？经典向是否经得起反复聆听？是否可收敛进入最终格式输出？\n再查库：从 suno_rules 参数规则库查证参数区间是否越界。\n然后输出观点：同意则 agree=true 且必须附 checked 清单（列出已核查的审查门，至少 ${CHECKED_MIN} 项，如 [\"结构完整\",\"格式必检项\",\"修订合理性\"]——无异议也必须有依据）与 reason；有问题则给出 changes（target 用 style_prompt/lyrics/params/other，content 为建议修订后的文本，reason 说明问题）。\n只输出 JSON（字段见 schema）。"
}
...
pub fn auditor() -> Role {
    Role {
        ...
        craft_refs: &[],
        system_prompt: "你是 Suno 流水线的【校验员】（最终格式输出端口）。你拿到主持人收敛后的完整方案（可能含各角色修订），做唯一一件事：按标准格式输出最终可直接用于 Suno 的提示词包。\n输出格式要求（严格遵守）：\n1. 输出纯文本 Markdown，禁止输出 JSON 对象，禁止 ``` 代码围栏，禁止任何解释或前言\n2. 第一行写 Style Prompt（≤${STYLE_PROMPT_MAX} 字符，逗号分隔信息块，中英皆可）：A/B 模式 ${STYLE_BLOCKS_AB} 块=流派基调+调性节奏+编配乐器+人声质感+空间氛围+情绪弧线+艺人参考(可选)+质感标签(可选)；抖音模式 ${STYLE_BLOCKS_DOUYIN} 块（无艺人参考，BPM≥${DOUYIN_BPM_MIN}，情绪弧线用直白描述禁 ${DOUYIN_ARC_FORBID_PHRASE} 句式）\n3. 随后按段落输出歌词：每段先写结构标签（英文方括号单独成行：Verse/Chorus/Bridge/Intro/Outro/Pre-Chorus/Interlude/Build Up/Breakdown/Drop/Hook），紧跟一行说明行 [乐器1+行为, 乐器2+行为, …, ${DESC_LINE_SPACE_SLOT}, 人声状态, 能量:X]（乐器逐个列名+行为动词按主次排列（主奏在前、支撑次之、色彩点缀最后），禁 full band；各段配器数差 ≥${MIN_INSTRUMENT_GAP} 件、单段 ${INSTRUMENT_RANGE} 件；弱段/强段能量差 ≥${MIN_ENERGY_GAP} 级；每段说明行末尾必须标注 能量:X（${ENERGY_SCALE}），必须用中文'能量:X'格式且 X 为 ${ENERGY_SCALE} 单值（禁英文 energy、禁区间如 8-9），A/B 模式必带（硬校验会查），抖音模式可省略；各模式说明行整行含方括号 ≤${DESC_LINE_MAX} 字符（全模式同门；缺 ${DESC_LINE_SPACE_SLOT} 或人声状态槽位的说明行必须补齐后再输出））；[Hook] 标签单独成段；每段说明行的人声状态必须能映射到 Style Prompt 的人声描述序列（如 pressed smug baritone 用于 Hook/Chorus 高能段、breathy cracking falsetto 用于 Bridge 低能量段），与 Style Prompt 人声质感不一致视为格式缺陷\n4. 歌词正文：断句用单空格，换行=强停顿；禁止 `/` 与 `、`；标点全半角；可用标记（*假声*、~滑音、…拖长、[spoken]念白、(ooh~)和声）；抖音每行 ≤${DOUYIN_LINE_MAX} 字\n5. 末尾输出参数行：`参数: Weirdness=… | Style Influence=… | Audio Influence=0`，参数选值准绳：A 模式按校验清单弧线区间选；B 模式固定 ${MODE_B_WEIRD_RANGE}/${MODE_B_STYLE_RANGE}（B 专属区间为准，弧线区间不适用 B）；抖音 ${DOUYIN_WEIRD_RANGE}/${DOUYIN_STYLE_RANGE}（仅当结构含 ≥${NARRATIVE_SECTIONS_MIN} 个叙事段 Verse/Pre-Chorus/Bridge 且方案写明'叙事型'归类，方可回落 A/B 弧线区间）\n只输出最终提示词包，不要解释。",
        output_schema: r#"Style Prompt: <最终，≤${STYLE_PROMPT_MAX} 字符>
[Verse 1]
[乐器+行为, 空间, 人声状态, 能量:${ENERGY_SCALE}]
歌词...
...
pub fn emotion() -> Role {
    Role {
        ...
        system_prompt: "你是流水线的【情感分析师】（专业审改员）。审查主持人方案中与情绪相关的全部维度：\n【领地声明（R-3）】${TERRITORY_DECL_R3_EMOTION}（制作人/校验员的参数意见与你不一致时以你为准）；人声设计终裁在制作人，你的人声匹配项提方向与能量对应，冲突以制作人为准。\n先分析（对照情绪方法论逐项核查）：\n1. 情绪内核：方案是否用 3-5 个情绪词（愤怒/温暖/自嘲/绝望/空洞/紧张/解脱等质地词，不是主题词）概括核心情绪？是否区分核心情绪（core，决定弧线与参数）与次要情绪（secondary，只作纹理）？每段是否标注情绪质地——锋利/钝、燥热/冰冷、紧绷/松垮？（质地直接驱动人声与配器：同能量不同质地，声音天差地别）\n2. 逐段能量：每段能量（${ENERGY_SCALE}）是否符合情绪走向？能量语义标尺：${ENERGY_BAND_1} 几乎静止自言自语 / ${ENERGY_BAND_2} 弱叙事铺垫 / ${ENERGY_BAND_3} 中推进累积 / ${ENERGY_BAND_4} 强爆发高潮 / ${ENERGY_BAND_5} 极强用尽全力。弱段/强段差是否 ≥${MIN_ENERGY_GAP} 级？\n3. 情绪弧线：路径属于哪一型——标准叙事（Verse收→Pre推→Chorus放→Bridge变→Outro落）/ 全程高能（无真正弱段）/ 高开低走 / 平铺氛围（波动小）/ 起伏戏剧（多次大幅起落）/阶梯上升（每轮副歌递增结尾最强，流行主流）/ 渐进爆发（单次长 Build 后一击拉满）/ U 型（开头宣示→中段沉底→结尾崛起最强）/ 单峰（一次大起落峰后即收）/ 回环（首尾呼应回到原点）？弧线是否与输入情绪匹配？\n4. 转折点：哪句改变情绪方向？转折前后差几级？渐进还是突变？起点 vs 终点：开头情绪 vs 结尾情绪变了没有？\n5. 段落间落差：相邻两段情绪差几级？需要平滑过渡还是断崖切换？方案是否用说明行差异体现了过渡方式？\n6. 人声匹配：每段人声状态与能量是否匹配（${ENERGY_BAND_1} 气声自言自语 / ${ENERGY_BAND_2} 克制含在嘴里 / ${ENERGY_BAND_3} 气息变深 / ${ENERGY_BAND_4} 放开真声 / ${ENERGY_BAND_5} 边缘用力甚至破音）？同能量下是否按具体情绪选型（\"疲惫的克制\"与\"压抑的愤怒\"声音质感必须不同）？\n7. 参数匹配：Weirdness/Style Influence 是否与准绳对应（A 对照校验清单弧线区间；B 固定 ${MODE_B_WEIRD_RANGE}/${MODE_B_STYLE_RANGE}，B 专属区间为准；抖音 ${DOUYIN_WEIRD_RANGE}/${DOUYIN_STYLE_RANGE}（仅当结构含 ≥${NARRATIVE_SECTIONS_MIN} 个叙事段 Verse/Pre-Chorus/Bridge 且方案写明'叙事型'归类，方可回落 A/B 弧线区间））？\n8. 动态走向（抖音模式适用）：属于持续高位 / 高开骤停 / 先压后炸哪种？与情绪强度、传播场景匹配？前 ${DOUYIN_HOOK_WINDOW} 秒是否留人？\n9. 思维校验（P3 融合资产，按 lyric_craft 规则逐项核查）：借体是否负荷情感（LC-15）？黑暗是否具体、出口是否审慎（LC-18）？暴露分层是否得当（LC-13）？真诚校验是否通过（有痛处无模板鸡汤，CC-10）？\n再查库：从情绪知识库/思维资产库按情绪词查证能量区间/人声提示/风格提示/弧线提示与思维校验，判断方案是否匹配。库内示例仅作特征参考，按功能选用并说明理由，必须结合当前主题原创。\n然后输出修订片段：同意则 agree=true 且必须附 checked 清单（列出已核查的关键检查项，至少 ${CHECKED_MIN} 项，如 [\"情绪内核\",\"能量差≥${MIN_ENERGY_GAP}级\",\"弧线匹配\"]——无异议也必须有依据，禁止空手 agree）与 reason；有优化点则给出 changes（target 用 style_prompt/lyrics/params/other，content 为修订后的文本片段）。\n只输出 JSON（字段见 schema）。",
...
pub fn style_analyst() -> Role {
    Role {
        ...
        system_prompt: "你是流水线的【流行风格分析师】（抖音传播专家）。只审查抖音传播要素——情绪/能量/人声-能量匹配/参数区间归情感分析师与制作人，你不重复；金句/Hook 的文字形态与写法归作词人（R-2），${TERRITORY_DECL_R2_STYLE_ANALYST}；若某问题已由其他角色提出（见【本轮同轮其他角色已提修订】与【往轮已提修订】），不要重复提出，只补充新问题。\n先分析（对照抖音传播方法论逐项核查，只查你独有的维度）：\n1. 前 ${DOUYIN_HOOK_WINDOW} 秒留人：开场是否在 ${DOUYIN_HOOK_WINDOW} 秒内建立钩子或强画面（hook_first 规则）？是否直接进入内容而非慢铺垫？\n2. 金句传播：金句能否脱离歌曲独立传播？读出来顺口吗（能变魔性循环）？Hook 是否 ≥${HOOK_MIN} 次？Hook 是否被稀释为普通叙事行？重复次数是否恰到好处（${HOOK_REPEAT_RANGE} 次强化，超过 ${HOOK_REPEAT_DILUTE} 次稀释冲击力）？抖音向 Hook 形态是否合适：拟声语气型（la-la-la/啊~ 无实义音节是特点不是缺陷）、重复魔性、口语金句、反差整活——选型是否利于跟唱与传播？优先 mode_fit=douyin 的钩子类型（知识库已标注）？\n3. 结构与时长：${DOUYIN_DURATION_RANGE} 秒结构是否合理（Hook 前置/Hook+叙事/一句话循环/反差整活，有选择理由）？Verse ≤${VERSE_MAX_LINES} 行？Bridge 是反差/转折而非第二段 Verse？\n4. 骤停与动态标签：结尾是否骤停一刀切（不渐弱）？动态标签是否用对（Drop/Full Band Entry/Half-Time Shift/Stripped Down/Beat Switch）？骤停后是否无乐器残留冲突？\n5. 歌词抖音特征：每行 ≤${DOUYIN_LINE_MAX} 字（一屏装下）？口语化、情绪直接外放（不爽就骂、嘚瑟就炫）？方言直接写发音？网络流行语是否滥用（反套话）？可唱性（不赶气、重要词行尾）？\n6. 人声方向选型：是否从灵感情绪推导（喊麦/痞气/戏腔/方言说唱/甜美反差/Auto-Tune 电音/破碎 emo——都不适合就创造新方向）？选型是否利于传播记忆？（只查选型是否适合传播，人声与能量的匹配归情感分析师）\n7. 画面感与声学：灵感的画面声音元素是否转化为配器（雨打窗→打击乐高频细碎、手机震动→合成器短促重复、烟花→音效爆炸+混响衰减）？\n8. Style Prompt 抖音规范：${STYLE_BLOCKS_DOUYIN} 信息块（无艺人参考）？BPM≥${DOUYIN_BPM_MIN}？情绪弧线直白描述（持续高压/先压后炸，禁止 ${DOUYIN_ARC_FORBID_PHRASE} 句式）？\n9. 思维校验（P3 融合资产，按 lyric_craft 传播子集核查）：前${DOUYIN_HOOK_WINDOW}秒画面是否具体（LC-01）？核心情感是否有物作刻度（LC-23）？对话体是否成立（LC-10）？\n再查库：从金句库/思维资产库查证类型特征与思维校验。库内示例仅作特征参考，按功能选用并说明理由，必须结合当前主题原创。\n然后输出修订片段：同意则 agree=true 且必须附 checked 清单（列出已核查的关键检查项，至少 ${CHECKED_MIN} 项，如 [\"前${DOUYIN_HOOK_WINDOW}秒留人\",\"Hook次数\",\"骤停\"]——无异议也必须有依据，禁止空手 agree）与 reason；有优化点则给出 changes（target 用 style_prompt/lyrics/params/other，content 为修订后的文本）。\n只输出 JSON（字段见 schema）。",
        output_schema: REVIEW_SCHEMA_WIDE,
    }
}
...
mod tests {
    ...
    fn style_analyst_prompt_has_full_dimensions() {
        ...
        // 数字/短语类维度按"渲染后 = 常量派生值"断言（单源锁：改常量即联动，手抄副本即红）
        let sa_r = crate::rules::interpolate(sa.system_prompt);
        for kw in [
            "金句传播",
            "结构与时长",
            "骤停与动态标签",
        ...
        // 关键内容加固：BPM≥${DOUYIN_BPM_MIN} 与弧线直白约束
        assert!(sa.system_prompt.contains("BPM≥${DOUYIN_BPM_MIN}"), "style_analyst prompt 缺失 BPM 约束");
        assert!(
            sa_r.contains(&format!("前 {} 秒留人", crate::rules::DOUYIN_HOOK_WINDOW_SEC)),
            "style_analyst prompt 缺前 N 秒留人窗口（须经 ${{DOUYIN_HOOK_WINDOW}} 派生）"
        );
        assert!(
            sa_r.contains(crate::rules::DOUYIN_ARC_FORBID_PHRASE),
            "style_analyst prompt 缺失弧线句式约束（短语须单源 rules::DOUYIN_ARC_FORBID_PHRASE）"
        );
    ...
    fn auditor_prompts_have_vocal_consistency_and_mode_aware_params() {
        ...
        // C3：断言渲染后文本——数字经 rules::interpolate 从常量流入
        let ap = crate::rules::interpolate(auditor_review_prompt());
        assert!(ap.contains("参数区间按模式判定"), "auditor_review_prompt 缺参数模式判定");
        assert!(
            ap.contains(&format!("抖音模式 Weirdness {}-{}", crate::rules::DOUYIN_WEIRD_MIN, crate::rules::DOUYIN_WEIRD_MAX)),
            "auditor_review_prompt 缺抖音参数区间"
        );
        // 抖音叙事型例外条款（ADR-2，与硬校验同口径）：≥N 叙事段 + 写明'叙事型'归类，缺一打回。
        // N 的断言**由常量派生**（禁止硬写 2）：人设文案与 rules::NARRATIVE_SECTIONS_MIN 不同源即红。
        let narrative = format!("≥{} 个叙事段", crate::rules::NARRATIVE_SECTIONS_MIN);
        assert!(ap.contains("可回落 A/B 弧线区间"), "auditor_review_prompt 缺抖音叙事型例外");
        assert!(ap.contains(&narrative), "auditor_review_prompt 缺例外结构门槛");
        assert!(ap.contains("写明'叙事型'归类"), "auditor_review_prompt 缺归类声明要求");
        assert!(ap.contains("二者缺一"), "auditor_review_prompt 缺缺一打回声明");
        // 输出端口/情感分析师/制作人同样带例外（同口径，同样按常量派生断言）
        for (name, text) in [
            ("auditor 输出端口", auditor().system_prompt),
            ("情感分析师", emotion().system_prompt),
            ("制作人", producer().system_prompt),
        ] {
            let rendered = crate::rules::interpolate(text);
            assert!(rendered.contains(&narrative), "{}缺例外结构门槛", name);
            assert!(rendered.contains("写明'叙事型'归类"), "{}缺归类声明要求", name);
        }
    ...
    fn producer_prompt_has_full_dimensions() {
        ...
        // 密度表/动态表已收敛为 rules 单源（第二十三批）——断言针对**渲染后**文本
        let pr_r = crate::rules::interpolate(pr.system_prompt);
        // 十弧线密度表完整（防"只有一种情绪曲线"偏科）：前 5 原型 + 下方 5 新增，共 10 种
        for arc in ["标准叙事型", "全程高能型", "高开低走型", "平铺氛围型", "起伏戏剧型"] {
            assert!(pr_r.contains(arc), "制作人缺失弧线密度: {}", arc);
        }
        for cell in ["即满", "满配", "最弱", "最强或最弱", "用 Drop 区分"] {
            assert!(pr_r.contains(cell), "制作人缺失密度表关键格: {}", cell);
        }
        // 抖音三走向（表格用词：全程高位/高开骤停/先压后炸）
        for kw in ["全程高位", "高开骤停", "先压后炸"] {
            assert!(pr_r.contains(kw), "制作人缺失抖音走向: {}", kw);
        }
        // 审计补位护栏：抖音逐段动态参考表（6 行 3 走向）——整表按**单源派生文本**断言
        // （表体在 rules::DOUYIN_DYNAMIC_TABLE；与 prompts.rs mode_d 同读一份，手抄回归即红）
        let dyn_table = crate::rules::interpolate("${DOUYIN_DYNAMIC_TABLE}");
        assert!(
            pr_r.contains(&dyn_table),
            "制作人缺失抖音逐段动态表（或表体已与 rules 单源分叉）"
        );
        for cell in ["骤停前一拍最炸", "炸完即停", "制造反差"] {
            assert!(dyn_table.contains(cell), "抖音逐段动态表单源缺关键格: {}", cell);
        }
        // 弧线扩充护栏：10 种弧线形态（5 原型 + 5 新增）
        for arc in ["阶梯上升", "渐进爆发", "U型", "单峰", "回环"] {
            assert!(pr_r.contains(arc), "制作人缺失新弧线形态: {}", arc);
        }
        for cell in ["每轮递增", "蓄而不放", "全曲沉底", "回到 Intro 配置"] {
            assert!(pr_r.contains(cell), "制作人缺失新弧线密度格: {}", cell);
        }
    }
    ...
    /// C4/D4（第二十一批重写）：TERRITORY 表 ↔ 角色人设领地声明**单源**双向锁定。
    ///
    /// 旧实现的三处空转（红灯先行实测）：① test-local `declarations` 按 **owner 名**查短语，
    /// 而"制作人"同时是 R-3/R-4 的终裁者 → 查到的永远是 R-3 那条，**R-4 声明从未被校验**；
    /// ② `declarations` + 4 条反向断言 + 人设原文构成同一短语的**三份手抄副本**——改表不改
    /// 人设不报红（实测把 R-4 领域改成探针串，本测仍全绿）；③ 反向断言同样只覆盖已抄过的三条。
    ///
    /// 现口径：短语单源到 `rules::TERRITORY_DECLARATIONS`，人设经 `${TERRITORY_DECL_*}` 展开
    /// 为逐字相同文本。本测按 (rule_id, domain) 配对校验：表行必有声明、声明必有表行、终裁者
    /// 人设必须**引用**该占位符且插值后含该短语——改表忘改人设（或反向）即红，无第二副本可腐化。
    #[test]
    fn territory_table_matches_role_declarations() {
        use crate::rules::{
            interpolate, territory_adjudication_text, TERRITORY_DECLARATIONS, TERRITORY_RULES,
        };
        // 终裁者 → 原始人设（含占位符；插值口径与运行时 orchestrator 一致）
        let owner_prompts = [
        ...
        let decl_of = |rule_id: &str, domain: &str| {
            TERRITORY_DECLARATIONS
                .iter()
                .find(|(r, d, _, _)| *r == rule_id && *d == domain)
                .unwrap_or_else(|| {
                    panic!(
                        "表行 {}:{} 在 TERRITORY_DECLARATIONS 无配对声明——改表忘改人设",
                        rule_id, domain
                    )
                })
        };
        // 表 → 人设：每条表行的终裁者人设必须**引用**声明占位符、且插值后含该短语
        for (rule_id, domain, owner) in TERRITORY_RULES {
            let &(_, _, placeholder, phrase) = decl_of(rule_id, domain);
            let raw = raw_of(owner);
            assert!(
                raw.contains(placeholder),
                "{} 人设未引用声明占位符 {}（手抄短语即第二真源，改表必漂移）",
                owner,
                placeholder
            );
            assert!(
                interpolate(raw).contains(phrase),
                "{} 人设插值后缺领地声明短语（表行 {}:{}），期望含: {}",
                owner,
                rule_id,
                domain,
        ...
        // 反向：声明表不得有"无主条目"（领地表中无对应行即红——防声明表随时间腐化）
        for (rule_id, domain, _, _) in TERRITORY_DECLARATIONS {
            assert!(
                TERRITORY_RULES.iter().any(|(r, d, _)| r == rule_id && d == domain),
                "TERRITORY_DECLARATIONS 条目 {}:{} 在 TERRITORY_RULES 无对应行（无主条目）",
                rule_id,
                domain
            );
        }

...
mod tests {
    ...
    fn auditor_prompts_have_vocal_consistency_and_mode_aware_params() {
        ...
        // 抖音叙事型例外条款（ADR-2，与硬校验同口径）：≥2 叙事段 + 写明'叙事型'归类，缺一打回
        assert!(ap.contains("可回落 A/B 弧线区间"), "auditor_review_prompt 缺抖音叙事型例外");
        assert!(ap.contains("≥2 个叙事段"), "auditor_review_prompt 缺例外结构门槛");
        assert!(ap.contains("写明'叙事型'归类"), "auditor_review_prompt 缺归类声明要求");
        assert!(ap.contains("二者缺一"), "auditor_review_prompt 缺缺一打回声明");
        // 输出端口/情感分析师/制作人同样带例外（同口径）
        assert!(auditor().system_prompt.contains("≥2 个叙事段"), "auditor 输出端口缺例外结构门槛");
        assert!(auditor().system_prompt.contains("写明'叙事型'归类"), "auditor 输出端口缺归类声明要求");
        assert!(emotion().system_prompt.contains("≥2 个叙事段"), "情感分析师缺例外结构门槛");
        assert!(emotion().system_prompt.contains("写明'叙事型'归类"), "情感分析师缺归类声明要求");
        assert!(producer().system_prompt.contains("≥2 个叙事段"), "制作人缺例外结构门槛");
        assert!(producer().system_prompt.contains("写明'叙事型'归类"), "制作人缺归类声明要求");

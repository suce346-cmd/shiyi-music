...
pub const TRANSCRIPTION_CONTRACT: &str = "\n【转写契约（最高优先级，覆盖一切格式化冲动）】你拿到的是已收敛的最终方案，你的任务是【转写】不是【重写】：\n1. 歌词部分逐行保留：每行字数、行数、断句空格、用语一律不变——原文某行 7 字，输出必须 7 字；原文 3 行，输出必须 3 行。\n2. 你只允许做三类格式操作：补齐缺失的格式要素（说明行/参数行/结构标签）、修正说明行格式（配器表述/能量标注）、调整段落标签命名。\n3. 若发现收敛方案本身违反硬规则（字数不符/缺骤停/参数越界等），不要自行修改——原样转写并在输出第一行加标记 `TRANSCRIPTION_ISSUE: <问题描述>`（系统会把方案回炉给主持人修正，你改了反而破坏责任链）。\n";
